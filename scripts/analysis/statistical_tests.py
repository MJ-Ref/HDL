...
def run_hypothesis_tests(protocols: Dict[str, Dict]) -> Dict[str, Any]:
    ...
    latent_protocols = [p for p in protocols.keys() if p.startswith('A') or p.startswith('E')]

    if text_protocols and latent_protocols:
        best_text = max(text_protocols, key=lambda p: np.mean(protocols[p]['successes']))
        best_latent = max(latent_protocols, key=lambda p: np.mean(protocols[p]['successes']))
    ...
    if 'P0' in protocols and 'P1' in protocols:
        ...
        results['Communication'] = {
            ...
            'p_value': p,
            'status': 'CONFIRMED' if sr_p1 > sr_p0 + 0.1 and p < 0.05 else 'NOT CONFIRMED'
        }
...
def main():
    ...
    if not episodes:
        print("No episodes found. Using demo data.")
