...
class LLMExperimentRunner:
    ...
    def _print_comparison(self, protocols: List[str]):
        ...
        print(f"{'='*60}")
        print(
            f"{'Protocol':<10} {'Success':<12} {'Partial':<10} {'Turns':<8} {'Bits':<8}"
        )
        print("-" * 50)
        ...
        if "P0" in self.all_results and "P1" in self.all_results:
            ...
            if p1_sr > p0_sr + 0.05:
                ...
                print(
                    f"  Slight benefit from communication: P1 ({p1_sr:.1%}) vs P0 ({p0_sr:.1%})"
                )
            else:
                print(
                    f"  No clear communication benefit: P1 ({p1_sr:.1%}) vs P0 ({p0_sr:.1%})"
                )
...
def main():
    ...
    parser.add_argument(
        "--protocols", type=str, help="Comma-separated protocols (e.g., P0,P1)"
    )
    parser.add_argument(
        "--n_episodes", type=int, default=10, help="Episodes per protocol"
    )
    parser.add_argument("--task", type=str, default="constraint_satisfaction")
    parser.add_argument(
        "--difficulty", type=str, default="easy", choices=["easy", "medium", "hard"]
    )
    parser.add_argument(
        "--base_seed", type=int, default=42, help="Base seed for paired episodes"
    )
