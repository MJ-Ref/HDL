...
class CIPHERExperimentRunner:
    ...
    def run_episode_with_cipher(
        ...
        for turn_idx in range(self.config.max_turns):
            ...
            # Prefill with soft-token context, then decode via KV cache so
            # the latent context remains active for all generated tokens.
            next_logits, past_key_values, base_seq_len = prefill_with_embeds(
                self._model,
                combined_embeds,
                combined_mask,
            )
            generated_ids = decode_with_cache(
                ...
                mask_dtype=combined_mask.dtype,
    ...
    def run(self) -> Dict[str, Any]:
        ...
        print(f"\n{'='*60}")
        print("E3: CIPHER Expected Embedding Experiment")
        print(f"{'='*60}")
